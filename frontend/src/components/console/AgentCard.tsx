...
import { Blade, BladeSection } from "../shell/Blade";
 ...
 * which is real. Devices & physical TX channels are the compact card's
 * summary plus a blade — the canvas's generic entity-inspection pattern
 * (Blade.tsx) — for the full per-channel table, since that's genuinely too
 * much detail to keep permanently visible in a dense agent list. The
 * canvas's AgentBlade also has a "Test Connection" action and a segregated
 * high-risk ARM block; neither exists as a real backend capability
 * (SDRAgentRecord has no such endpoint), so they're omitted rather than
 * added as controls that would do nothing.
 */
    ...
    >
      ...
      >
        ...
        {fieldRow(
          "devices",
          deviceIds.length > 0
            ? `${deviceIds.length} device(s) · ${agent.capabilities.length} TX ch`
            : "0",
        )}
      ...
      <div style={{ margin: "0 11px 9px" }}>
        ...
        >
          Devices &amp; channels ({agent.capabilities.length}) ▸
        </button>
      </div>
      {bladeOpen && (
        <Blade
          ...
          sub={`${agent.mode} · ${agent.status} · last seen ${new Date(agent.last_seen_at).toLocaleString()}`}
        ...
        >
          <BladeSection
            ...
            rows={[
              ...
              { k: "last seen", v: new Date(agent.last_seen_at).toLocaleString() },
              { k: "host", v: <span style={{ color: "var(--ink-3)" }}>not reported yet</span> },
              { k: "clock", v: <span style={{ color: "var(--ink-3)" }}>not reported yet</span> },
            ]}
          />
          <div
            style={{
              ...
              <tbody>
                {agent.capabilities.map((cap) => (
                  <tr key={`${cap.device_id}:${cap.channel_index}`}>
                    ...
                    <td style={{ padding: "4px 11px", borderBottom: "1px solid var(--line)" }}>
                      {cap.tunable_ranges_hz.map(ghz).join(", ")}
                    </td>
                    <td style={{ padding: "4px 11px", borderBottom: "1px solid var(--line)" }}>
