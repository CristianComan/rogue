"""Tests for DeepwaveAIR7311Adapter (M10, ADR-010) against a fake
SoapyDevice — no real SoapySDR bindings or hardware needed, matching
CLAUDE.md §10's simulation default and this pass's explicit "code now,
verify later" scope. Mirrors test_x440_adapter.py's coverage shape since
both adapters share their vendor-agnostic logic
(agents.common.sdr_adapter_base.StreamingSDRAdapter).
"""
...
from agents.common.air7311_adapter import (
...
class _FakeSoapyDevice:
    ...
    def discover_channel(self, channel: int) -> ChannelCapabilityReadback:
        return ChannelCapabilityReadback(
            tunable_ranges_hz=[(70e6, 6e9)], max_usable_bandwidth_hz=100e6, max_sample_rate_hz=125e6
        )
...
def make_recording(**overrides: object) -> IQRecording:
    kwargs: dict[str, object] = {
        "version": 1,
        "metadata_object_key": "recordings/air7311-test.sigmf-meta",
        "data_object_key": "recordings/air7311-test.sigmf-data",
...
def make_composite_channel(
    ...
    kwargs: dict[str, object] = {
        ...
        "role": RfLinkRole.C2,
        "emission_id": uuid4(),
        "center_frequency_hz": 5_200_000_000.0,
...
) -> tuple[DeepwaveAIR7311Adapter, _FakeSoapyDevice]:
    device = _FakeSoapyDevice()
    adapter = DeepwaveAIR7311Adapter(
...
async def test_discover_reads_back_real_device_ranges(tmp_path: Path) -> None:
    ...
    assert capabilities[0].max_usable_bandwidth_hz == 100e6
...
async def test_configure_calls_device_with_window_frequency(tmp_path: Path) -> None:
    ...
    assert device.configured[CHANNEL]["freq_hz"] == 5_200_000_000.0
...
async def test_emergency_stop_never_raises_even_if_device_end_burst_fails(tmp_path: Path) -> None:
    class _FailingDevice(_FakeSoapyDevice):
        def end_burst(self, channel: int) -> None:
