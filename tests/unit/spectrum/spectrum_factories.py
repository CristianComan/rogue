"""Minimal, valid object builders for spectrum tests.

Deliberately not shared with tests/unit/domain/factories.py: pytest's
rootdir-based import (no __init__.py in either directory) makes each test
directory its own top-level import namespace, so cross-directory imports
aren't reliable when a test subset runs in isolation — same rationale as
tests/unit/persistence/persistence_factories.py.
"""
...
def make_mission(rf_links: list[DroneRfLink], trajectory: Trajectory | None = None) -> DroneMission:
    return DroneMission(
        name="recon-1",
        platform=Platform(
